...
import com.ims.oversea.eplatonframework.business.facade.spuserbean.*;
...
public class SPuserbeanBizAction extends EPlatonBizAction
{
  ...
  public IEvent act(IEvent event)
  {
    ...
    {
      event = event;
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPuserbeanBizAction.act() start");

      SPuserbeanManagementSBHome spuserbeanHome = null;
      SPuserbeanManagementSB iSPuserbeanManagementSB = null;

      SPuserbeanManagementSBLocalHome spuserbeanLocalLocalHome = null;
      SPuserbeanManagementSBLocal iSPuserbeanManagementSBLocal = null;
      ...
      {
        spuserbeanHome =
           (SPuserbeanManagementSBHome)JNDIService.getInstance()
                                          .lookup(SPuserbeanManagementSBHome.class);
        iSPuserbeanManagementSB = spuserbeanHome.create();

        event = this.doAct(iSPuserbeanManagementSB, event);
      }
      else if( TCFConstants.BEAN_LOCAL_TYPE.equals(call_bean_type) )
      {
        spuserbeanLocalLocalHome =
           (SPuserbeanManagementSBLocalHome)JNDIService.getInstance()
                                          .lookup(SPuserbeanManagementSBLocalHome.class);
        iSPuserbeanManagementSBLocal = spuserbeanLocalLocalHome.create();

        event = this.doAct(iSPuserbeanManagementSBLocal, event);
      }
      else{
        spuserbeanHome =
           (SPuserbeanManagementSBHome)JNDIService.getInstance()
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPuserbeanBizAction success");
    }
    catch(Exception _e)
    {
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPuserbeanBizAction error:[EFWK0035]"+_e.toString() );
    }
    finally{
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPuserbeanBizAction.act() end");
    }
