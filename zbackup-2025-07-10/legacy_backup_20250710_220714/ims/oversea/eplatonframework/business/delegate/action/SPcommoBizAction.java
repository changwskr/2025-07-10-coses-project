...
import com.ims.oversea.eplatonframework.business.facade.spcommo.*;
...
public class SPcommoBizAction extends EPlatonBizAction
{
  ...
  public IEvent act(IEvent event)
  {
    ...
    {
      event = event;
      LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,(EPlatonEvent)event,"=================================================SPcommoBizAction.act() start");
      ...
      LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,(EPlatonEvent)event,"SPcommoBizAction success");
    }
    catch(Exception _e)
    {
      SET_SPerror(TCFConstantErrcode.EACT002,TCFConstantErrcode.EACT002_MSG + "," + this.getClass().getName() + " exception");
      LOGEJ.getInstance().eprintf(TCFConstants.TCF_LOG_LEVEL,(EPlatonEvent)event,_e);
      LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,(EPlatonEvent)event,"SPcommoBizAction error:[EFWK0035]"+_e.toString() );
    }
    finally{
      LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,(EPlatonEvent)event,"=================================================SPcommoBizAction.act() end");
    }
