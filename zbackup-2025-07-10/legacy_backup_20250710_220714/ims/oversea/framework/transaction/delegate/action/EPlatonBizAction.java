  ...
  protected IEvent doAct(Object target, IEvent event)
  {
    ...
    try
    {
      if( tpsvcinfo.getTpfq().equals(TCFConstants.SERVER_SERVER_INTERCHANGE_LOC) )
      {
         ...
         */
        event = TPMSutil.getInstance().TPSrecv((EPlatonEvent)event);
        TPMSutil.getInstance().TPSDBrecv((EPlatonEvent)event);

        /*
         * 실제 각 업무시스템 즉 facade단의 세션빈에 대한 메소드를 호출한다.
         * 만약 TPSrecv() 단계에서 에러가 발생하면 업무시스템으로 들어가지는 않는다.
         * 왜냐하면 트랜잭션 번호등이 여기서 채번되기 때문이다.
         */
        Method meth = (target.getClass()).getMethod(methodName,new Class[]{EPlatonEvent.class});
        LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"start to invoke system method-[" + meth.toString() + "]" );
        if ( !isErr((EPlatonEvent)event) ){
          ...
          LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"fail to invoke system method because of ETPM008");
        }
      }
      else{
        /*
      ...
      if( tpsvcinfo.getTpfq().equals(TCFConstants.SERVER_SERVER_INTERCHANGE_LOC) ){
        ...
        TPMSutil.getInstance().TPSsend((EPlatonEvent)event);
      }
    ...
    catch(Exception _e)
    {
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"EPlatonBizAction error:[EACT001](MethodName=" + methodName + ") 호출 에러");
      SET_SPerror(TCFConstantErrcode.EACT001,TCFConstantErrcode.EACT001_MSG + "["+ _e.toString() +"]");
