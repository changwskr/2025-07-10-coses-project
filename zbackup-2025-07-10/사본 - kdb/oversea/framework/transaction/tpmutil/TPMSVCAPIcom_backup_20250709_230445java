 ...
 */
  ...
  public void TPSDBrecv(EPlatonEvent event) {
    ...
    try{
      ...
      LOGEJ.getInstance().printf(4,event,"1-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );
      switch( TPinfo() )
      {
        case -1: // 에러발생시
          return;
        ...
        case 0:  // Active Transaction
          LOGEJ.getInstance().printf(4,event,"2-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );

          gtx = TPSsuspend();
          if( gtx == null ){
            LOGEJ.getInstance().printf(5,event,"TPSDBrecv()-TPSsuspend() 실패");
            return;
          }
          else{
            LOGEJ.getInstance().printf(5,event,"--->>> TPSDBrecv()-TPSsuspend() 성공");
            resume_mode = 1;
          }
          break;
        default:
          LOGEJ.getInstance().printf(5,event,"TPSDBrecv()-TPinfo() nothing");
          return;
      }
...
LOGEJ.getInstance().printf(4,event,"3-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );

      if( resume_mode == 1 ){
          if( TPSresume(gtx) ){
            LOGEJ.getInstance().printf(5,event,"------>>>> TPSresume() 성공");
          }
          else{
            LOGEJ.getInstance().printf(5,event,"TPSresume() 실패");
          }
      }
  ...
  public void TPSDBsend(EPlatonEvent event,long interval_seconds)
  {
    ...
    try{
      ...
      LOGEJ.getInstance().printf(4,event,"1-----Transaction TPinfo mode : ["
      ...
      switch( TPinfo() )
      {
        ...
        case 1:  // Marked Rollback
          ...
          gtx = TPSsuspend();
          if( gtx == null ){
            LOGEJ.getInstance().printf(5,event,"TPSDBsend()-TPSsuspend() 실패");
            return;
          }
          else{
            LOGEJ.getInstance().printf(5,event,"--->>> TPSDBsend()-TPSsuspend() 성공");
          }
          break;
        case 0:  // Active Transaction
          gtx = TPSsuspend();
          ...
          break;
        default:
          LOGEJ.getInstance().printf(5,event,"TPSDBsend()-TPinfo() nothing");
          return;
      }
  ...
  public EPlatonEvent TPSrecv(EPlatonEvent event)
  {
    ...
    try
    {
      ...
      if( action_name == null ){
        ...
        LOGEJ.getInstance().printf(3,event,"ERROR-Cannot get action_class_name");
      }
      else{
        ...
        if ( operation_class == null ){
          ...
          LOGEJ.getInstance().printf(3,event,"ERROR-Cannot get operation_class_name");
        }
        else{
          // 호출할 시스템 클래스명
          ...
          if( method_name == null ){
            ...
            LOGEJ.getInstance().printf(3,event,"ERROR-Cannot get operation_method_name");
          }
          else{
            ...
            if( parametertype == null ) {
              ...
              LOGEJ.getInstance().printf(3,event,"ERROR-Cannot get parametertype");
            }
            else{
      ...
      * 400 : ATM - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      ...
      if( tpsvcinfo.getTpfq() != null && tpsvcinfo.getTpfq() != "" ){
        ...
        LOGEJ.getInstance().printf(3,event,"ERROR-not set tpfq:100,200,300...800");
      }
    ...
    //System.out.println(tpsvcinfo.getReqName() + " TPSrecv()-hostseq "+tpsvcinfo.getHostseq()+"-"+commonDTO.getTransactionNo() );
