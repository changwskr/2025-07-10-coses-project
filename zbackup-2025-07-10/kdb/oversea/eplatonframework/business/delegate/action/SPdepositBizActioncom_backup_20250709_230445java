...
import com.kdb.oversea.eplatonframework.business.facade.cashCard.*;
...
import com.kdb.oversea.framework.transaction.delegate.action.EPlatonBizAction;
import com.kdb.oversea.eplatonframework.business.facade.spdeposit.*;
...
public class SPdepositBizAction extends EPlatonBizAction
{
  ...
  private String call_bean_type = "remote";

  /**
   * 생성자함수
   */
  public SPdepositBizAction()
  {
  }
  ...
  public IEvent act(IEvent event)
  {
    ...
    {
      event = event;
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPdepositBizAction.act() start");

      SPdepositManagementSBHome spdepositHome = null;
      SPdepositManagementSB iSPdepositManagementSB = null;

      SPdepositManagementSBLocalHome spdepositLocalLocalHome = null;
      SPdepositManagementSBLocal iSPdepositManagementSBLocal = null;
      ...
      if( "remote".equals(call_bean_type) )
      {
        spdepositHome =
           (SPdepositManagementSBHome)JNDIService.getInstance()
                                          .lookup(SPdepositManagementSBHome.class);
        iSPdepositManagementSB = spdepositHome.create();

        event = this.doAct(iSPdepositManagementSB, event);
      }
      else if( "local".equals(call_bean_type) )
      {
        spdepositLocalLocalHome =
           (SPdepositManagementSBLocalHome)JNDIService.getInstance()
                                          .lookup(SPdepositManagementSBLocalHome.class);
        iSPdepositManagementSBLocal = spdepositLocalLocalHome.create();

        event = this.doAct(iSPdepositManagementSBLocal, event);
      }
      else{
        spdepositHome =
           (SPdepositManagementSBHome)JNDIService.getInstance()
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPdepositBizAction success");
    }
    catch(Exception _e)
    {
      SET_SPerror("EFWK0035",this.getClass().getName()+ ".act():"+_e.toString());
      LOGEJ.getInstance().eprintf(5,(EPlatonEvent)event,_e);
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPdepositBizAction error:[EFWK0035]"+_e.toString() );
    }
    finally{
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPdepositBizAction.act() end");
    }
