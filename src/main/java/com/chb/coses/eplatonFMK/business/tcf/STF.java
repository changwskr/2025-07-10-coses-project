...
/*
 CREATE TABLE COMMON_INFO (
  ...
  CONSTRAINT PK_LIVE_TRANSFER PRIMARY KEY (BUSINESS_DATE)
  )
/
...
public class STF implements ISTF {
  ...
  public boolean STF_SPinit()
  {
    try{
       ...
       ************************************************************************/
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"gitxInfoflag set :: ");
      if( tpsvcinfo.getTpfq() == null ){
        STF_SPerror("EFWK0050",this.getClass().getName()+ ".STF_SPinit():: TPFQ IS NULL");
      }
      ...
      */
       ...
       * Base Currency
       ************************************************************************/
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"Base Currency set ::  ");
      if( (businessDate=STF_SPgetbasecurrency()) == null ){
        STF_SPerror("EFWK0039",this.getClass().getName()+ ".STF_SPinit()::get base curreycy error");
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"error to get base curreycy");
      }
      else
  ...
  public String STF_SPgetbasecurrency()
  {
    String aa = null;
    try {
      EPlatonDelegateDAO dao = new  EPlatonDelegateDAO();
      aa = dao.queryForBaseCurreycy(commonDTO.getBankCode());
    }
    catch(Exception ex){
      STF_SPerror("EFWK0038",this.getClass().getName()+ ".STF_SPgetbasecurrency():"+ex.toString());
      LOGEJ.getInstance().eprintf((EPlatonEvent)eplevent,ex);
      return aa;
    }
    return(aa);
