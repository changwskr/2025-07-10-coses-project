...
import com.chb.coses.eplatonFMK.business.helper.CommonUtil;
import com.chb.coses.eplatonFMK.business.helper.EJBUtils;
import com.chb.coses.eplatonFMK.business.dao.EPlatonDelegateDAO;
...
public class ETF implements IETF {
  ...
  public ETF(SessionContext tx,int tx_start_info){
    this.tx = tx;
  ...
  public EPlatonEvent execute(EPlatonEvent pevent){
    try{
      ...
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPinit] start");
      ETF_SPinit();
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPinit] end");

      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPmiddle] start");
      ETF_SPmiddle();
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPmiddle] end");

      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPend] start");

      ETF_SPend();
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"=================================[ETF_SPend] end");
  ...
  public boolean ETF_SPmiddle()
  {
    try{
      ...
      ETF_SPwebtxtimer();
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"ending : [" + tpsvcinfo.getTx_timer() + "]"  );
  ...
  public boolean ETF_SPend()
  {
    try{
      ...
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[ETF_SPdbOutLog() START]");
      if( !ETF_SPdbOutLog(eplevent) ){
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"ETF_SPdbOutLog() error");
      }
      else{
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"ETF_SPdbOutLog() success");
      }
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[ETF_SPdbOutLog() END] (true)");


      /***************************************************************************
       * 트랜잭션에대한 로깅정보를 관리한다.
       **************************************************************************/
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[ETF_SPcommonLog START]");

      if( !ETF_SPcommonLog() ){
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"ETF_SPcommonLog() error");
      }
      else{
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"ETF_SPcommonLog() success");
      }
      LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[ETF_SPcommonLog() END] (true)");
       ...
       * 일단은 구조만 가지고 간다. (향후변경)
       **************************************************************************/
      if( this.giTXInfoflag == 1 && tpsvcinfo.getTpfq().equals("200") ){
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[TProllback/TPcommit START]"+giTXInfoflag+tpsvcinfo.getTpfq());
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"Business Error Code : [" + tpsvcinfo.getErrorcode() + "] " + tpsvcinfo.getErrorcode().charAt(0));

        switch( tpsvcinfo.getErrorcode().charAt(0) )
        {
          case 'E' :
            LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"Call - TProllback()");
            if( TPMSVCAPI.getInstance().TProllback(this.tx) )
              LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"success");
            else{
            ...
            break;
          case 'I' :
            LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"Call - TPcommit()");
            break;
          default :
            LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"Call - TProllback() - default");
        ...
        LOGEJ.getInstance().printf((EPlatonEvent)eplevent,"==================[TProllback/TPcommit END]-("+this.giTXInfoflag +")");
      }
  ...
  public void setETF_SPtxinfo(int offset){
   ...
   public void setSessionContext(SessionContext tx)
   {
     this.tx=tx;
   }
