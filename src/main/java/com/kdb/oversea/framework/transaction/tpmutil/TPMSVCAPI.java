...
import com.kdb.oversea.framework.transaction.constant.TCFConstants;
 ...
 * spcashcard.xml 정보
 ...
 <spcashcard-callmethod01>
        <bizaction-class>com.kdb.oversea.eplatonframework.business.delegate.action.SPcashcardBizAction</bizaction-class>
        <transactionable>Y</transactionable>
        ...
        <operation-class>com.kdb.oversea.cashCard.business.facade.CashCardManagementSBBean</operation-class>
...
*/
  ...
  public boolean TProllback(SessionContext ctx,EPlatonEvent event)
  {
    try{
      ...
      ex.printStackTrace();
      LOGEJ.getInstance().printf(1,event,"tprollback()에러가 발생합니다.-" + ex.toString() );
      LOGEJ.getInstance().eprintf(1,event,ex);
      return false;
    }
    return true;
  }
  ...
  public void TPSDBrecv(EPlatonEvent event) {
    ...
    try{
      ...
      LOGEJ.getInstance().printf(1,event,"1-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );
      switch( TPinfo() )
      {
        case -1: // 에러발생시
          return;
        ...
        case 0:  // Active Transaction
          LOGEJ.getInstance().printf(1,event,"2-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );

          gtx = TPSsuspend();
          if( gtx == null ){
            LOGEJ.getInstance().printf(1,event,"TPSDBrecv()-TPSsuspend() 실패");
            return;
          }
          else{
            LOGEJ.getInstance().printf(1,event,"--->>> TPSDBrecv()-TPSsuspend() 성공");
            resume_mode = 1;
          }
          break;
        default:
          LOGEJ.getInstance().printf(1,event,"TPSDBrecv()-TPinfo() nothing");
        return;
      }
      ...
      if( !dao.DB_INSERT_tpminlog(event) ){
        LOGEJ.getInstance().printf(1,event,"TPSDBrecv() 에러");
      }
      else{
        LOGEJ.getInstance().printf(1,event,"TPSDBrecv() 성공");
      }

      LOGEJ.getInstance().printf(1,event,"3-----Transaction TPinfo mode : [" + TxHelper.status2String(TPinfo()) +"] tpfq:[" +event.getTPSVCINFODTO().getTpfq()+"]"  );

      if( resume_mode == 1 ){
        if( TPSresume(gtx) ){
          LOGEJ.getInstance().printf(1,event,"------>>>> TPSresume() 성공");
        }
        else{
          LOGEJ.getInstance().printf(1,event,"TPSresume() 실패");
        }
      }
      ...
      LOGEJ.getInstance().eprintf(1,(EPlatonEvent)event,ex);
      ex.printStackTrace();
    }
  ...
  public void TPSDBsend(EPlatonEvent event,long interval_seconds)
  {
    ...
    try{
      ...
      LOGEJ.getInstance().printf(1,event,"1-----Transaction TPinfo mode : ["
      ...
      switch( TPinfo() )
      {
        ...
        case 1:  // Marked Rollback
          ...
          gtx = TPSsuspend();
          if( gtx == null ){
            LOGEJ.getInstance().printf(1,event,"TPSDBsend()-TPSsuspend() 실패");
            return;
          }
          else{
            LOGEJ.getInstance().printf(1,event,"--->>> TPSDBsend()-TPSsuspend() 성공");
          }
          break;
        case 0:  // Active Transaction
          gtx = TPSsuspend();
          ...
          break;
        default:
          LOGEJ.getInstance().printf(1,event,"TPSDBsend()-TPinfo() nothing");
        return;
      }
      ...
      if( !dao.DB_INSERTtpmoutlog(event,interval_seconds) ){
        LOGEJ.getInstance().printf(1,event,"TPSDBsend() 에러");
      }
      else{
        LOGEJ.getInstance().printf(1,event,"TPSDBsend() 성공");
      }
  ...
  public EPlatonEvent TPSrecv(EPlatonEvent event)
  {
    ...
    try
    {
      ...
      if( action_name == null ){
        tpsvcinfo.setErrorcode("EFWK0001");
        tpsvcinfo.setError_message("Cannot get action_class_name");
        LOGEJ.getInstance().printf(5,event,"ERROR-Cannot get action_class_name");
      }
      else{
        ...
        if ( operation_class == null ){
          tpsvcinfo.setErrorcode("EFWK0002");
          tpsvcinfo.setError_message("Cannot get operation_class_name");
          LOGEJ.getInstance().printf(5,event,"ERROR-Cannot get operation_class_name");
        }
        else{
          // 호출할 시스템 클래스명
          ...
          if( method_name == null ){
            tpsvcinfo.setErrorcode("EFWK0003");
            tpsvcinfo.setError_message("Cannot get operation_method_name");
            LOGEJ.getInstance().printf(5,event,"ERROR-Cannot get operation_method_name");
          }
          else{
            ...
            if( parametertype == null ) {
              tpsvcinfo.setErrorcode("EFWK0004");
              tpsvcinfo.setError_message("Cannot get parametertype");
              LOGEJ.getInstance().printf(5,event,"ERROR-Cannot get parametertype");
            }
            else{
      ...
      * 1차적인 데이타검증을 실시한다.
      ...
      if( commonDTO.getEventNo() == null )
      {
        ...
        LOGEJ.getInstance().printf(5,event,"EPlatonCommonDTO.eventno is not set");
      }
      ...
      * 400 : ATM - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      ...
      if( tpsvcinfo.getTpfq() != null && tpsvcinfo.getTpfq() != "" ){
        ...
        LOGEJ.getInstance().printf(5,event,"ERROR-not set tpfq:100,200,300...800");
      }
      ...
      * DB세션과 거래번호가 정상적으로 채번되었는지 확인한다.
      ...
      else
      {
        ...
        LOGEJ.getInstance().printf(6,event,"==============================================================================");
        LOGEJ.getInstance().printf(6,(EPlatonEvent)event,
                                   "TPSrecv()-"+com.chb.coses.foundation.utility.Reflector.objectToString(event));
        LOGEJ.getInstance().printf(6,event,"TPSrecv()-error");
      }
    }
    catch(Exception ex){
      ex.printStackTrace();
      ...
      LOGEJ.getInstance().eprintf(5,event,ex);
      LOGEJ.getInstance().printf(6,event,"==============================================================================");
      LOGEJ.getInstance().printf(6,(EPlatonEvent)event,"TPSrecv()-"+com.chb.coses.foundation.utility.Reflector.objectToString(event));
      LOGEJ.getInstance().printf(6,(EPlatonEvent)event,"TPSrecv()-error code : [" + tpsvcinfo.getErrorcode() + "]");
      try{
        con.close();
      }
    ...
    if( tpsvcinfo.getHostseq().equals("********") )
    {
      tpsvcinfo.setErrorcode("EDL998");
      tpsvcinfo.setError_message("TPSrecv HOSTSEQ GET ERROR");
      LOGEJ.getInstance().printf(6,event,"TPSrecv()-error-hostseq is ******:errcode["+tpsvcinfo.getErrorcode()+"]" );
    }

    LOGEJ.getInstance().printf(6,event,"==============================================================================");
    LOGEJ.getInstance().printf(6,(EPlatonEvent)event,
                               "TPSrecv()-"+com.chb.coses.foundation.utility.Reflector.objectToString(event));
    LOGEJ.getInstance().printf(6,(EPlatonEvent)event,"error code : [" + tpsvcinfo.getErrorcode() + "]");
    return event;
  }
  ...
  public EPlatonEvent TPSsend(EPlatonEvent event)
  {
    EPlatonCommonDTO commonDTO = (EPlatonCommonDTO)event.getCommon();
    TPSVCINFODTO tpsvcinfo = (TPSVCINFODTO)event.getTPSVCINFODTO();
    LOGEJ.getInstance().printf(6,event,"TPSsend()-errorcode:["+tpsvcinfo.getErrorcode()+"]");
    LOGEJ.getInstance().printf(6,(EPlatonEvent)event,"TPSsend()-return data["+com.chb.coses.foundation.utility.Reflector.objectToString(event) +"]");
    LOGEJ.getInstance().printf(6,event,"==============================================================================\n\n");
