...
import com.kdb.oversea.eplatonframework.business.facade.lcommon.*;
...
public class SPcommonBizAction extends EPlatonBizAction
{
  ...
  public IEvent act(IEvent event)
  {
    ...
    {
      event = event;
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPcommonBizAction.act() start");

      SPcommonManagementSBHome spcommonHome = null;
      SPcommonManagementSB iSPcommonManagementSB = null;

      SPcommonManagementSBLocalHome spcommonLocalLocalHome = null;
      SPcommonManagementSBLocal iSPcommonManagementSBLocal = null;
      ...
      {
        spcommonHome =
           (SPcommonManagementSBHome)JNDIService.getInstance()
                                          .lookup(SPcommonManagementSBHome.class);
        iSPcommonManagementSB = spcommonHome.create();

        event = this.doAct(iSPcommonManagementSB, event);
      }
      else if( "local".equals(call_bean_type) )
      {
        spcommonLocalLocalHome =
           (SPcommonManagementSBLocalHome)JNDIService.getInstance()
                                          .lookup(SPcommonManagementSBLocalHome.class);
        iSPcommonManagementSBLocal = spcommonLocalLocalHome.create();

        event = this.doAct(iSPcommonManagementSBLocal, event);
      }
      else{
        spcommonHome =
           (SPcommonManagementSBHome)JNDIService.getInstance()
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPcommonBizAction success");
    }
    catch(Exception _e)
    {
      SET_SPerror("EFWK0035",this.getClass().getName()+ ".act():"+_e.toString());
      LOGEJ.getInstance().eprintf(5,(EPlatonEvent)event,_e);
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPcommonBizAction error:[EFWK0035]"+_e.toString() );
    }
    finally{
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPcommonBizAction.act() end");
    }
