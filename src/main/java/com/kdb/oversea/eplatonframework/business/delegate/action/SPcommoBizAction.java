...
import com.kdb.oversea.eplatonframework.business.facade.spcommo.*;
...
public class SPcommoBizAction extends EPlatonBizAction
{
  ...
  public IEvent act(IEvent event)
  {
    ...
    {
      event = event;
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPcommoBizAction.act() start");

      SPcommoManagementSBHome spcommonHome = null;
      SPcommoManagementSB iSPcommoManagementSB = null;

      SPcommoManagementSBLocalHome spcommonLocalLocalHome = null;
      SPcommoManagementSBLocal iSPcommoManagementSBLocal = null;
      ...
      {
        spcommonHome =
           (SPcommoManagementSBHome)JNDIService.getInstance()
                                          .lookup(SPcommoManagementSBHome.class);
        iSPcommoManagementSB = spcommonHome.create();

        event = this.doAct(iSPcommoManagementSB, event);
      }
      else if( "local".equals(call_bean_type) )
      {
        spcommonLocalLocalHome =
           (SPcommoManagementSBLocalHome)JNDIService.getInstance()
                                          .lookup(SPcommoManagementSBLocalHome.class);
        iSPcommoManagementSBLocal = spcommonLocalLocalHome.create();

        event = this.doAct(iSPcommoManagementSBLocal, event);
      }
      else{
        spcommonHome =
           (SPcommoManagementSBHome)JNDIService.getInstance()
      ...
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPcommoBizAction success");
    }
    catch(Exception _e)
    {
      SET_SPerror("EFWK0035",this.getClass().getName()+ ".act():"+_e.toString());
      LOGEJ.getInstance().eprintf(5,(EPlatonEvent)event,_e);
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"SPcommoBizAction error:[EFWK0035]"+_e.toString() );
    }
    finally{
      LOGEJ.getInstance().printf(1,(EPlatonEvent)event,"=================================================SPcommoBizAction.act() end");
    }
