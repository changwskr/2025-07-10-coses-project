...
import com.ims.oversea.eplatonframework.business.facade.splogej.*;
import com.ims.oversea.framework.transaction.helper.EJBUtils;
...
public class TPMSutil extends AbstractTPMSVCAPI implements ITPMSVCAPI
{
  private ICommonManagementSB commonManagementSB;
  private IReferenceManagementSB referenceManagementSB;

  private static TPMSutil instance;

  public static synchronized TPMSutil getInstance(String ip,String port) {
    if (instance == null) {
      try{
        instance = new TPMSutil(ip,port);
      }
  ...
  public TPMSutil(String ip,String port) throws Exception{
    try{
      url = "t3://"+ip+":"+port;
  ...
  public static synchronized TPMSutil getInstance() {
    if (instance == null) {
      try{
        instance = new TPMSutil();
      }
      catch(Exception igex)
      {
        //System.out.println(igex);
  ...
  public TPMSutil() throws Exception{
    try{
   ...
   *
   *     String configFileName = TCFConfig.getInstance().getElement(Constants.BIZDELEGATE_TAG).getTextTrim();
...
*/
  ...
  {
    String configFileName = TCFConfig.getInstance().getElement(TCFConstants.BIZDELEGATE_TAG).getTextTrim();
  ...
  public boolean TPbegin(UserTransaction tx,int second)
  {
    try{
      ...
      tx.setTransactionTimeout(second-TCFConstants.GLOBAL_TRANSACTION_MINUS_SECONDS);
      tx.begin();
  ...
  public boolean TPbegin(String second) throws Exception
  {
    try{
      ...
      tm.setTransactionTimeout(CommonUtil.Str2Int(second)-TCFConstants.GLOBAL_TRANSACTION_MINUS_SECONDS );
    }
    catch(Exception ex){
      ex.printStackTrace();
      return false;
  ...
  public void TPSDBrecv(EPlatonEvent event) {
    ...
    SPlogejManagementSBLocalHome splogejLocalLocalHome = null;
    SPlogejManagementSBLocal iSPlogejManagementSBLocal = null;
    ...
    try{
      ...
      switch( TPinfo() )
      {
        case -1: // 에러발생시
          return;
        case 4:  // Rolledback
          LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,event,"this transaction has been already rollback-maybe timeout");
          try{
            splogejLocalLocalHome =
                (SPlogejManagementSBLocalHome)JNDIService.getInstance()
                .lookup(SPlogejManagementSBLocalHome.class);
            iSPlogejManagementSBLocal = splogejLocalLocalHome.create();
            event = iSPlogejManagementSBLocal.execute((EPlatonEvent)event,'i');
          }catch(Exception igex){}
          return;
        ...
        case 0:  // Active Transaction
          ...
          gtx = TPSsuspend();
          if( gtx == null ){
            event.getTPSVCINFODTO().setErrorcode(TCFConstantErrcode.ETPM021);
            event.getTPSVCINFODTO().setError_message(TCFConstantErrcode.ETPM021_MSG);
          ...
          break;
        default:
          LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,event,"TPSDBrecv()-TPinfo() ["+TxHelper.status2String(TPinfo())+"]");
        return;
      }
      ...
      if( resume_mode == 1 ){
        if( TPSresume(gtx) ){
          ...
          event.getTPSVCINFODTO().setErrorcode(TCFConstantErrcode.ETPM022);
          event.getTPSVCINFODTO().setError_message(TCFConstantErrcode.ETPM022_MSG);
    ...
    try{
      ...
      switch( TPinfo() )
      {
        ...
        case 4:  // Rolledback
          try{
            splogejLocalLocalHome =
                ...
                .lookup(SPlogejManagementSBLocalHome.class);
            iSPlogejManagementSBLocal = splogejLocalLocalHome.create();
            event = iSPlogejManagementSBLocal.execute((EPlatonEvent)event,'o');
          }catch(Exception igex){}
          return;
        case 6:  // No Transaction
          break;
        ...
        case 0:  // Active Transaction
          ...
          break;
        default:
          LOGEJ.getInstance().printf(TCFConstants.GENERAL_LOG_LEVEL,event,"TPSDBsend()-TPinfo() ["+TxHelper.status2String(TPinfo())+"]");
          return;
      }
   ...
   * @return
   */
  public EPlatonEvent TPSrecv_new(EPlatonEvent event)
  {
    Connection con = null;
    String hostseq = null;
    ...
    try
    {
       ...
       *************************************************************************/
      // 호출할 시스템 명
      tpsvcinfo.setSystem_name(TPMSutil.getInstance().TPgetcallsystemname(tpsvcinfo.getReqName() ) );
      // 호출할 시스템으로 라우팅할 action 클래스명
      String action_name = TPMSutil.getInstance().TPgetactionclassname(event);
      if( action_name == null ){
        ...
        String operation_class = TPMSutil.getInstance().TPgetoperationclassname(event);
        if ( operation_class == null ){
          ...
          // 호출할 시스템 클래스의 메소드명
          String method_name = TPMSutil.getInstance().TPgetinvokemethodname(event);
          if( method_name == null ){
            ...
            String parametertype = TPMSutil.getInstance().TPgetparametertypename(event);
      ...
      * WAS로부터 하나의 DB세션을 구한후 거래번호를 채번한다
      ...
      try{
        ICommonManagementSB commonManagementSB = EJBUtils.getCommonEJB();
        if( (businessDate = commonManagementSB.getBusinessDate(commonDTO.getBankCode())) == null ){
          TPM_SPerror(event,TCFConstantErrcode.ETPM016,TCFConstantErrcode.ETPM016_MSG);
          LOGEJ.getInstance().printf(TCFConstants.FATAL_LOG_LEVEL,(EPlatonEvent)event,"TPSrecv()::get business date error");
        }
      }
      catch(Exception ex){
        ex.printStackTrace();
        tpsvcinfo.setErrorcode(TCFConstantErrcode.ETPM017);
        tpsvcinfo.setError_message(TCFConstantErrcode.ETPM017_MSG);
        LOGEJ.getInstance().printf(TCFConstants.TPM_LOG_LEVEL,event,"not to get business");
      }

      try{
        IReferenceManagementSB reference = EJBUtils.getReferenceEJB();
        transactionNo = reference.getTransactionNumber(commonDTO.getBankCode(), businessDate);
        commonDTO.setTransactionNo(transactionNo);
        hostseq = transactionNo;
      }
      catch(Exception ex){
        ex.printStackTrace();
        tpsvcinfo.setErrorcode(TCFConstantErrcode.ETPM018);
        tpsvcinfo.setError_message(TCFConstantErrcode.ETPM018_MSG);
        LOGEJ.getInstance().printf(TCFConstants.TPM_LOG_LEVEL,event,"not to get transactionno");
      }
      ...
      tpsvcinfo.setSTF_intime( tpsvcinfo.getSystemInTime() );
      tpsvcinfo.setSTF_outtime( tpsvcinfo.getSystemInTime() );
      tpsvcinfo.setETF_intime( tpsvcinfo.getSystemInTime() );
      tpsvcinfo.setETF_outtime( tpsvcinfo.getSystemInTime() );
      tpsvcinfo.setBTF_intime( tpsvcinfo.getSystemInTime() );
      tpsvcinfo.setBTF_outtime( tpsvcinfo.getSystemInTime() );
  ...
  public EPlatonEvent TPSrecv(EPlatonEvent event)
  {
    ...
    try
    {
      ...
      event.getMessageParameter().setRule_intime(CommonUtil.GetSysTime());
      event.getMessageParameter().setRule_operation_class_name(TCFConstants.RULE_THING_INITIAL_VAL);
      event.getMessageParameter().setRule_operation_method_name(TCFConstants.RULE_THING_INITIAL_VAL);
      event.getMessageParameter().setRule_outtime(CommonUtil.GetSysTime());
      event.getMessageParameter().setThing_intime(CommonUtil.GetSysTime());
      event.getMessageParameter().setThing_operation_class_name(TCFConstants.RULE_THING_INITIAL_VAL);
      event.getMessageParameter().setThing_operation_method_name(TCFConstants.RULE_THING_INITIAL_VAL);
      event.getMessageParameter().setThing_outtime(CommonUtil.GetSysTime());
        ...
        if ( operation_class == null ){
          ...
          if( method_name == null ){
            ...
            if( parametertype == null ) {
              ...
              tpsvcinfo.setCdto_name( parametertype );
              // EPlatonEvent 객체의 전달범위는 각업무단 시스템의 대표모듈까지 전달 된다.
              // 그래서 CDTO에 서버의 정보를 셋팅해서 보낸다.
              // 이것은 DTO객체를 RULE단까지 보내기 위한 방법으로 제시한다.
              // 모든 CDTO는 TCFDTO을 상속받아서 사용한다.
              TPSinputDTO(event);
            }
          }
        }
      ...
      * 100 : Server System - Server System 연동 => 거래번호 신규채번 => OrgSeq은 그대로 유지
      * 200 : WAF - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      * 300 : native - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      * 400 : ATM - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      * 500 : internet bank - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
      * 600 : batch - Server System 연동 => 거래번호를 신규채번 => OrgSeq은 신규채번 번호로 대입
       */
      if( tpsvcinfo.getTpfq() != null && tpsvcinfo.getTpfq() != "" ){
        if( !tpsvcinfo.getTpfq().equals(TCFConstants.SERVER_SERVER_INTERCHANGE_LOC) ){
          tpsvcinfo.setHostseq(TCFConstants.HOSTSEQ_INITIAL_STR);
      ...
      * WAS로부터 하나의 DB세션을 구한후 거래번호를 채번한다
      ...
      con = DBService.getInstance().getConnection(
          TCFConfig.getInstance().getElement("connection_pool").getChild("name").getTextTrim());
  ...
  public void TPSinputDTO(EPlatonEvent event)
  {
    try{
      //----------------------------------------------------------------------
      // TCFDTO 정보를 가지고 완서 해당 시스템과 메소드명 그리고 트랜잭션 번호를 기입한다
      //----------------------------------------------------------------------
      ((TCFDTO)event.getResponse()).setMethod(event.getTPSVCINFODTO().getOperation_method());
      ((TCFDTO)event.getResponse()).setSystem(event.getTPSVCINFODTO().getSystem_name());
      ((TCFDTO)event.getResponse()).setTransaction_no(event.getTPSVCINFODTO().getHostseq());
    }
    catch(Exception ex){
    }
  }
  ...
  public String TPMinterval(String starttime,String endtime)
  {
    String psStartTimer=starttime;
    String psEndTimer=endtime;
    String ss=null;
    String mm=null;
    String ee=null;
    String rr=null;
    ...
    /***********************************************************************/
      ...
      rr=psStartTimer.substring(6,8);

      //100미리세컨드를 기준
      startmsec = 100 - CommonUtil.Str2Int(rr);
    ...
    /***********************************************************************/
      ...
      rr=psEndTimer.substring(6,8);

      endmsec = CommonUtil.Str2Int(rr);
      endsec = CommonUtil.Str2Int(ss)*60*60 + CommonUtil.Str2Int(mm)*60 + CommonUtil.Str2Int(ee);

      return (endsec - startsec) + "." + ( startmsec + endmsec );
    }
    catch(Exception ex){
      return "0";
    }
  ...
  private void TPM_SPerror(EPlatonEvent event,String errorcode,String message)
  {
  ...
  public static void main(String args[]) {
    System.out.println(TPMSutil.getInstance().TPMinterval("135737933","135738100"));
  }
