...
import com.ims.eplaton.eplatonFKC.transfer.TPMSVCINFO;
...
public class TPSVCINFODTO extends DTO
{
  ...
  // 클라이언트에서 셋팅 정보
  // "com.ims.eplaton.eplatonFKC.business.delegate.action.CashCardBizAction"
  ...
  public static void prnttpmsvcinfo(EPlatonEvent event)
  {
    ...
    for( int i = 0 ; i < al.size() ; i ++ )
    {
      ...
      LOGEJ.getInstance().printf(1,event,"| " + i + " " + tm.getCall_service_name() + " " + tm.getCall_tpm_in_time() + " " + tm.getCall_tpm_out_time() + " " + tm.getError_code() + " " + "010" + " " + "LOC"  );
    }
  }
