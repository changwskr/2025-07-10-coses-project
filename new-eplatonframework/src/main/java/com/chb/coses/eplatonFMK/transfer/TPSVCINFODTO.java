...
import com.chb.coses.eplatonFMK.transfer.TPMSVCINFO;

/**
 * TPM Service Info DTO
 * Spring 기반으로 전환된 TPM 서비스 정보 DTO
 * 
 * @author unascribed
 * @version 1.0
 */
...
public class TPSVCINFODTO extends DTO {
    ...
    @JsonProperty("tpmsvcinfolist")
    private ArrayList<TPMSVCINFO> tpmsvcinfolist;
    ...
    @JsonProperty("cdto_name")
    private String cdto_name;

    @JsonProperty("action_name")
    private String action_name;
    // 클라이언트에서 셋팅 정보
    // "com.chb.coses.eplatonFMK.business.delegate.action.CashCardBizAction"
    ...
    // step 3. doAct()에서는 action클래스에서 생성된 세션빈과 파라미터(EPlatonEvent)를 셋팅해서 각 업무시스템을
    // 호출한다.
    ...
    @JsonProperty("trclass")
    private String trclass; // 마감전후구분

    @JsonProperty("web_timeout")
    private String web_timeout;

    @JsonProperty("web_intime")
    private String web_intime;

    @JsonProperty("web_outtime")
    ...
    public ArrayList<TPMSVCINFO> getTpmsvcinfolist() {
    ...
    public void setTpmsvcinfolist(ArrayList<TPMSVCINFO> tpmsvcinfolist) {
